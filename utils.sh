...
build_rv() {
	...
	for build_mode in "${build_mode_arr[@]}"; do
		...
		if [ -n "$microg_patch" ]; then
			patched_apk="${TEMP_DIR}/${app_name_l}-${version_f}-${arch_f}-${build_mode}.apk"
		else
			patched_apk="${TEMP_DIR}/${app_name_l}-${version_f}-${arch_f}.apk"
		fi
		if [ -n "$microg_patch" ]; then
		...
		if [ "$build_mode" = apk ]; then
			local apk_output="${BUILD_DIR}/${app_name_l}-v${version_f}-${arch_f}.apk"
		...
		local module_output="${app_name_l}-${version_f}-${arch_f}-${build_mode}.zip"
